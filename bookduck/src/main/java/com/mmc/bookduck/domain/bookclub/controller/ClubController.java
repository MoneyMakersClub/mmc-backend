package com.mmc.bookduck.domain.bookclub.controller;

import com.mmc.bookduck.domain.bookclub.dto.request.ClubCreateRequestDto;
import com.mmc.bookduck.domain.bookclub.dto.response.*;
import com.mmc.bookduck.domain.bookclub.service.ClubInviteService;
import com.mmc.bookduck.domain.bookclub.service.ClubService;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.tags.Tag;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import java.util.List;

@Tag(name = "Club", description = "북클럽 생성 및 초대 관련 API")
@RestController
@RequestMapping("/clubs")
@RequiredArgsConstructor
public class ClubController {
    private final ClubService clubService;
    private final ClubInviteService clubInviteService;

    @Operation(summary = "클럽 생성", description = "새로운 북클럽을 생성하고 초대 링크를 발급합니다.")
    @PostMapping
    public ResponseEntity<ClubCreateResponseDto> createClub(@Valid @RequestBody ClubCreateRequestDto requestDto) {
        return ResponseEntity.ok(clubService.createClub(requestDto));
    }

    @Operation(summary = "초대 코드 검증", description = "초대 링크를 통해 접근 시 초대 코드 유효성을 검증하고 클럽 정보를 반환합니다.")
    @GetMapping("/invite/validate")
    public ResponseEntity<ClubInviteValidationResponseDto> validateInvite(@RequestParam String inviteCode) {
        return ResponseEntity.ok(clubInviteService.validateInvite(inviteCode));
    }

    @Operation(summary = "초대 수락", description = "유효한 초대 링크를 통해 북클럽에 가입합니다.")
    @PostMapping("/invite/accept")
    public ResponseEntity<ClubInviteAcceptResponseDto> acceptInvite(@RequestParam String inviteCode) {
        return ResponseEntity.ok(clubInviteService.acceptInvite(inviteCode));
    }

    @Operation(summary = "가입된 클럽 목록 조회", description = "현재 로그인한 사용자가 속한 모든 클럽 목록과, 각 클럽별 읽지 않은 글 수를 반환합니다.")
    @GetMapping("/joined")
    public ResponseEntity<List<ClubJoinedResponseDto>> getJoinedClubs() {
        return ResponseEntity.ok(clubService.getJoinedClubs());
    }
}
